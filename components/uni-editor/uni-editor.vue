		...
		<view class="page-body">
			<view class='wrapper'>
				...
				<editor id="editor" class="ql-container" placeholder="开始输入..." showImgSize showImgToolbar showImgResize
				 @statuschange="onStatusChange" :read-only="readOnly" @ready="onEditorReady" @input="handleInput"	>
				</editor>
			</view>
		</view>
		...
		methods: {
			...
			insertImage() {
				...
				uni.chooseImage({
					count: 1,
					success: (res) => {
						this.uploadImage(res.tempFilePaths[0]).then((data)=>{
							console.log(data)
							this.editorCtx.insertImage({
								src: store.apiUrl+data,
								alt: '图像',
								width:'100%',
								height:'auto',
								success: function() {
			...
			handleInput(res){
				const {html, text, delta}=res.detail;
				this.html=html
				console.log(html)
			}
		},
