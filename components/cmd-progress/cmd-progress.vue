    ...
    computed: {
      ...
      setCircleTrailStyle() {
        ...
        return `stroke-dasharray: ${len - (gapDeg||0)}px, ${len}px;
				...
				transition: stroke-dashoffset 0.06s ease 0s, stroke-dasharray 0.06s ease 0s, stroke 0.06s, stroke-width 0.01s ease 0.06s;`
      },
      /**
       * 圈进度
       */
      ...
      setCircle() {
        ...
        if (this.status == 'success' || this.setProgress >= 100 || this.strokeColor) {
          currentColor = this.strokeColor || '#549EF3'
        }
        let svgToBase =
